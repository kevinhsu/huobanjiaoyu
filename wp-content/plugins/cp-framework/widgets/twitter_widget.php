...
?>
  ...
  <label for="<?php echo $this->get_field_id('title'); ?>">
	  <?php _e('Widget Title:','crunchpress');?>
   ...
   <p>
   <?php _e('Please visit this link first','crunchpress');?>
    <a href="https://dev.twitter.com/apps/new">https://dev.twitter.com/apps/new</a> <?php _e('it will take to page where you can make twitter app that will help you to get some details that you need to activate your twitter widget.','crunchpress');?>
  ...
  <label for="<?php echo $this->get_field_id('consumer_secret'); ?>">
	 <?php _e('Consumer Secret Key','crunchpress');?>
	</label> 
	<br/ >
  ...
  <label for="<?php echo $this->get_field_id('user_secret'); ?>">
	   <?php _e('User Secret Token','crunchpress');?>
  </label>
  <br/ >
  ...
  <label for="<?php echo $this->get_field_id('username_widget'); ?>">
	  <?php _e('User Name (without twitter url for example: http://www.twitter.com/Envato write only Envato): ','crunchpress');?>
  </label>
  <br/ >
  ...
  <label for="<?php echo $this->get_field_id('num_of_tweets'); ?>">
	  <?php _e('Number of Tweets','crunchpress');?>
  </label>
  <br/ >
...
<?php
  ...
  function update($new_instance, $old_instance)
  {
	...
	function widget($args, $instance)
	{
		...
		echo '<div id="tweets_crunchpress" class="tweetss">';
		...
		echo '<script type="text/javascript">jQuery(document).ready(function ($) { $(".twitter-'.$counter_twi.'").bxSlider({mode:"fade", auto:"true", speed:"slow", controls:false, pager:false,});});</script>';
		?>
		<ul class="twitter-<?php echo $counter_twi;?>" id="footer-tweet">
		<?php
		if($tweets <> ''){
			$counter_twitter = 0;
				...
				?>
					<li>
						<div class="tweets"> 
							...
							<a href="http://twitter.com/<?php echo $this_tweet->user->screen_name; ?>/status/<?php echo $this_tweet->id_str; ?>"></a>
						</div>
					</li>
			<?php
			}
