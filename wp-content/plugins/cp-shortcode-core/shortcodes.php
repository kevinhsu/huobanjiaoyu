...
*/
	...
	//Courses shortcode
	add_shortcode('courses', 'cp_courses_shortcode');
	//Events shortcode
	add_shortcode('events', 'cp_events_shortcode');
	//Lesson shortcode
	add_shortcode('lesson', 'cp_lesson_shortcode');
	...
	function cp_donation_shortcode($atts,$content = null){
		...
		$donation_currency = get_themeoption_value('donation_currency','general_settings');
		...
		<section class="donate-page">
			<div class="donate-form">
			  <form action="https://www.paypal.com/cgi-bin/webscr" class="donate-form-area">
				...
				<ul>
					<li>
						<input type="radio" class="radio" name="amount" id="radio_1" value="50" />
						<label for="radio_1">					
							<span class="show">50</span>
							<span class="show-hover">50</span>
						</label>
					</li>
					<li>
						<input type="radio" class="radio" name="amount" id="radio_2" value="150" />
						<label for="radio_2">					
							<span class="show">150</span>
							<span class="show-hover">150</span>
						</label>
					</li>
					<li>
						<input type="radio" class="radio" name="amount" id="radio_3" value="300" />
						<label for="radio_3">					
							<span class="show">300</span>
							<span class="show-hover">300</span>
						</label>
					</li>
					<li>
						<input type="radio" class="radio" name="amount" id="radio_4" value="350" />
						<label for="radio_4">					
							<span class="show">350</span>
							<span class="show-hover">350</span>
						</label>
					</li>
					<li>
						<input type="radio" class="radio" name="amount" id="radio_5" value="400" />
						<label for="radio_5">					
							<span class="show">400</span>
							<span class="show-hover">400</span>
						</label>
					</li>
					<li>
						<input type="radio" class="radio" name="amount" id="radio_6" value="450" />
						<label for="radio_6">					
							<span class="show">450</span>
							<span class="show-hover">450</span>
						</label>
					</li>
				</ul>				
				<ul>
				...
				<input name="submit" type="image" value="continue" class="donate-btn-submit">
			</form>
		</div>
	</section>';
	...
	function cp_heading_shortcode($atts,$content = null){
		...
		$head_t_colr = 'style="color:'.$title_color.'"';
		$head_sub_colr = 'style="color:'.$sub_color.'"';
		$head_sub_bg = 'style="background-color:'.$sub_color.'"';
		
		if($style == 'style1'){
			$heading_html = '<div class="cp_heading_style" style="text-align:'.$align.'"><'.$size.' '.$head_t_colr.'>'.$title.'</'.$size.'><p '.$head_sub_colr.'>'.$description.'</p>
				<div class="dots"><span '.$head_sub_bg.'></span><span '.$head_sub_bg.'></span><span '.$head_sub_bg.'></span></div>
			</div>';
		}else{
			$heading_html = '<div class="cp_heading_style cp_style_2" style="text-align:'.$align.'"><'.$size.' '.$head_t_colr.'>'.$title.'</'.$size.'><p '.$head_sub_colr.'>'.$description.'</p></div>';
		}
		return $heading_html;
			...
			if($event_id <> ''){
				...
				if(isset($EM_Event)){
					...
					if($today < $start_date_time){
						...
						$event_html = '<div class="circle-time">
						<style>
						#countdown-'.$event_counter.' .time_circles > div > h4,
						#countdown-'.$event_counter.' .time_circles > div > span{color:'.$color.' !important;}
						</style>
	...
	function cp_event_counter_box_shortcode($atts,$content = null){
		...
		if(class_exists('EM_Events')){
			...
			if( ($rowno < $limit || empty($limit)) && ($event_count >= $offset || $offset === 0) ) {
				...
				$hour_end = date('H',$EM_Event->end_time);
				$min_end = date('i',$EM_Event->end_time);
				$sec_end = date('s',$EM_Event->end_time);
	...
	function cp_checklist_shortcode($atts,$content = null){
		...
		$counter_checklist++;		
		//HTML Markup
		return '<style scoped>.list-style-cp-'.$counter_checklist.' li:before{color:'.$iconcolor.';content:"'.$icon_aw.'"}</style><div class="list-style-cp-'.$counter_checklist.' list-style cp-list-style">
		'.$content.'
		</div>';
	...
	function cp_pricing_price_shortcode($atts,$content = null){
		...
		//HTML Markup
		return '<div class="cp_price_rate">
				<h2>'.$currency.$price.'</h2><p>/'.$time.'</p>
			</div>';
	}
	
	//Pricing Table Col Start
	function cp_pricing_column_shortcode($atts,$content = null){
		return '<div class="table-body"><ul>'.do_shortcode($content).'</ul></div>';
	}
	...
	function cp_pricing_footer_shortcode($atts,$content = null){
		...
		//HTML Markup
		return '<a class="btn-style" href="'.$link.'">'.do_shortcode($content).'</a></div>';
	}
	...
	function cp_testimonials_shortcode($atts,$content = null){
		...
		$html_testimonial .= '<div class="cp-testimonials"><div class="testimonial-box-1"> <div class="container">
	...
	function cp_testi_shortcode($atts,$content = null){
		...
		if($type == 'default'){
			...
			$html_testi .= '<div class="single-testimonial_cp cp_white_color">
            <blockquote> <q> <i class="fa fa-quote-left"></i>'.do_shortcode($content).'</q> </blockquote>
            <div class="frame-box2">
              '.$picture.'
			  <strong class="name"><a  target="'.$target.'" href="'.$link.'">'.$name.'</a></strong><strong class="title">'.$company.'</strong>
          </div></div>';
			...
			<div class="frame">
					<a href="'.$link.'"><img src="'.$picture.'" alt="'.$name.'"></a>
				<div class="single-testimonial_cp">';
				...
				<div class="holder">
					<div class="frame-box">
						<div class="frame">
							...
							<blockquote><q>'.do_shortcode($content).'</q></blockquote>
				</div>
			</li>';
		}
		return $html_testi;
	...
	function cp_courses_shortcode($atts,$content = null){
		...
		//Get All Courses In Tag ID 38 [Media]
		...
		<div class="box">
			<h2>'.$title.'</h2>
				...
				foreach ($my_courses as $mycourse){
					//Get Featured Image src of Course
					$cp_image = get_the_post_thumbnail($mycourse->ID, array(80,80));
					$short_content = substr($mycourse->post_content, 0, 50);
					$course_title = substr($mycourse->post_title, 0, 20);
					$course_detail_xml = get_post_meta($mycourse->ID, 'course_detail_xml', true);
					...
					$html .= '
						<li>
							<div class="thumb">
								<a href="'.$mycourse->guid.'">'.$cp_image.'</a>
							</div>
							<div class="text">
								<h4><a href="'.$mycourse->guid.'">'.$course_title.'</a></h4>
								<p>'.$short_content.'</p>
								<span class="rate"><small>'.$currency.'</small>'.$sale_price.'</span>
							</div>
						</li>';
					}
					$html .= '</ul></div>';
	...
	//Events Shortcode
	function cp_events_shortcode($atts,$content = null){
		...
		if(class_exists('EM_Events')){
			...
			global $EM_Events,$bp;				
				$limit = $number_of_events;	//Default limit
				$page = ( !empty($_GET['pno']) ) ? $_GET['pno']:1;
				$offset = ( $page > 1 ) ? ($page-1)*$limit : 0;
				$order = ( !empty($_REQUEST ['order']) ) ? $_REQUEST ['order']:'ASC';
				$EM_Events = EM_Events::get( array('cat'=>$event_cat_id, 'group'=>'this','scope'=> $event_type, 'limit' => $number_of_events, 'order' => 'DESC') );
            ...
            '.$title.'
                <ul class="event-galley">';
					foreach ( $EM_Events as $event ) {
						...
						// Getting Single Event ID and Passing it to em_get_event function
						$event_id = $event->ID;		
						...
						if(class_exists('EM_Events')){
							...
							$location_summary = '';
							$location_summary = $EM_Event->get_location()->name;
						...
						//Get Featured Image src of Course
						$cp_image = get_the_post_thumbnail($event->ID, array(80,80));
						$short_content = substr($event->post_content, 0, 95);
						$event_title = substr($event->post_title, 0, 20);
						$event_html .= '
							<li>
								<div class="thumb">
									<a href="'.$event->guid.'">'.$cp_image.'</a>
								</div>
								<div class="text">
									<p>'.$day.'- '.$month.'- '.$year.'</p>
									<h4>'.$event_title.'</h4>
									<a href="'.$event->guid.'" class="tags">'.$location_summary.'</a>									
								</div>
							</li>';
					}
			$event_html .= '</ul></div>';
	...
	// Lesson ShortCode
	function cp_lesson_shortcode($atts,$content = null){
		...
		//Get Lesson Array By ID
		$lesson = get_post($lesson_id); 
		$short_content = substr($lesson->post_content, 0, 350);
		$cp_image = get_the_post_thumbnail($lesson->ID, array(570,300));
		$cp_education_class = new cp_education_class;
		//HTML Markup
		$html .= '
				<div class="box lesson">
					<h2>'.$title.'</h2>
					'.$cp_education_class->print_lesson_thumbnail($lesson->ID, array(570,300)).'
					<div class="text">
						<h3 class="pull-left"><a href="'.$lesson->guid.'">'.$lesson->post_title.'</a></h3>
						<p>'.strip_tags($short_content).'</p>
					</div>
				</div>';
		return $html;
	}
	...
	function cp_title_shortcode($atts,$content = null){
		...
		//HTML Markup
		return '<'.$size.' class="cp-heading-full" style="color:'.$color.';">'.do_shortcode($content).'</'.$size.'>';
	...
	function cp_map_shortcode($atts){
		...
		$html = '<div class="cp-map-containter"><script type="text/javascript" src="https://maps.googleapis.com/maps/api/js?sensor=true"></script>';
	...
	function cp_slide_shortcode($atts ,$content = null)
	{
		...
		if($type == 'image'){
			$html_slider = '<li><a href="'.$link.'" target="'.$target.'"><img src="'.$content.'" /></a></li>';
		}else{
	...
	function cp_accordion_shortcode($atts, $content = null)
	{
		...
		wp_register_script('cp-accor-crunch', CP_PATH_URL.'/frontend/js/jquery.accordion.js', false, '1.0', true);
		wp_enqueue_script('cp-accor-crunch');
		wp_enqueue_style('cp-accor-crunch',CP_PATH_URL.'/frontend/css/accordion.css');
