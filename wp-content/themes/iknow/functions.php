<?php 
	...
	*/
	...
	//include_once(CP_FW.	'/extensions/newsletter.php'); 				// Super object function
	...
	include_once(CP_FW.	'/cp_course_management.php');					// Course Management control panel
	...
	include_once(CP_FW. '/extensions/captcha/default/cp_default_captcha.php'); // Custom Captcha Plugin
	...
	include_once(CP_FW. '/extensions/widgets/cp_courses_widget.php'); // Custom Coureses Widget
	
	
	//Mega Menu Start Here
	$mega_menu_layout = get_themeoption_value('mega_menu_layout','general_settings');
	if($mega_menu_layout == 'enable'){
		include_once(CP_FW. '/extensions/cp_menu_walker.php'); 							// Register pagination plugin
	}
	//Mega Menu Ends Here
	
	include_once(CP_FW. '/extensions/plugins.php'); 				// Custom Or External Plugins
	include_once(CP_FW. '/extensions/contact_submit.php'); // Registered CP Header style
...
?>	
		...
		<div class="page-heading">
			<div class="container">						
				<h2><?php if(is_single()){ echo esc_attr(get_the_title());}else{ esc_attr(woocommerce_page_title());};?></h2>
...
<?php
	   ...
	   public static function register ( $wp_customize ) {
		  ...
		  $wp_customize->add_section( 'mytheme_options', 
			 array(
				...
				'description' => esc_html__('Allows you to customize some example settings for crunchpress.', 'crunchpress'), //Descriptive tooltip
			 ) 
		  );
		  ...
		  $wp_customize->add_control( new WP_Customize_Color_Control( //Instantiate the color control class
			 ...
			 array(
				'label' => esc_html__( 'Link Color & Button Color', 'crunchpress' ), //Admin-visible name of the control
 ...
 // Register your custom function to override some LayerSlider data
	...
	function cp_admin_notice_framework() { ?>
		<div class="updated">
			<p><strong><?php esc_html_e( 'Please install theme required plug-ins to use all functionalities of theme', 'crunchpress' ); ?></strong> - <?php esc_html_e('in case of deactivating the theme required plug-ins you may not able to use theme extra functionality.','crunchpress');?></p>
		</div>
		<?php
	}
	...
	function cp_admin_notice_maintenance_mode() { ?>
		<div class="error">
			<p><strong><?php esc_html_e( 'Theme has activated maintenance mode!', 'crunchpress' ); ?></strong> - <?php esc_html_e('Please turn it off from Cp Theme Panel > General Settings > Maintenance Mode Settings > Maintenance Mode (On/Off).','crunchpress');?></p>
		</div>
		<?php
	}
	...
	function cp_wp_title( $title, $sep ) {
		...
		if ( $paged >= 2 || $page >= 2 ) {
			$title = "$title $sep " . sprintf( esc_html__( 'Page %s', 'crunchpress' ), max( $paged, $page ) );
		}
