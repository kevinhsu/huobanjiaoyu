	 ...
	 <?php esc_html_e('Select Category:','crunchpress');?> 
	  <select id="<?php echo esc_attr($this->get_field_id('select_category')); ?>" name="<?php echo esc_attr($this->get_field_name('select_category')); ?>" class="widefat">
		...
		foreach ( get_category_list_array('product_cat') as $category){ ?>
                    <option <?php if($c == $category->slug){echo 'selected';}?> value="<?php echo esc_attr($category->slug);?>" >
						...
						query_posts($args);
						 ...
						 $counter_new = 0;
							...
							<li> 
								...
								<div class="bottom_sec"> <span class="price"><?php echo esc_attr($regular_price);?></span><span><a href="<?php echo esc_url(get_permalink());?>"><?php esc_html_e('Add to cart','crunchpress');?><i class="icon-shopping-cart"></i></a></span></div>
							</li>
						<?php 
						endwhile;
						}?>
