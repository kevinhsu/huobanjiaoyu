...
?>
  ...
  <label for="<?php echo esc_attr($this->get_field_id('title')); ?>">
	 <?php esc_html_e('Title:','crunchpress');?>  
	  <input class="title" size="40" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr($title); ?>" />
  </label>
  </p>
  <p>
  <label for="<?php echo esc_attr($this->get_field_id('recent_event_category')); ?>">
	  <?php esc_html_e('Select Category:','crunchpress');?>
	  <select id="<?php echo esc_attr($this->get_field_id('recent_event_category')); ?>" name="<?php echo esc_attr($this->get_field_name('recent_event_category')); ?>" style="width:225px">
		<?php
		
				foreach ( get_category_list_array('event-categories') as $category){ ?>
                    <option <?php if($recent_event_category == $category->term_id){echo 'selected';}?> value="<?php echo esc_attr($category->term_id);?>" >
  ...
  <label for="<?php echo esc_attr($this->get_field_id('number_of_events')); ?>">
	  <?php esc_html_e('Number of events','crunchpress');?>
	<input class="title" size="5" id="<?php echo esc_attr($this->get_field_id('number_of_events')); ?>" name="<?php echo esc_attr($this->get_field_name('number_of_events')); ?>" type="text" value="<?php echo esc_attr($number_of_events); ?>" />
  </label>
  </p>
<?php
  }
  ...
  function update($new_instance, $old_instance)
  {
	...
	<!-- Links Start -->          
		<div class="upcoming-events">
				...
				if($counter_new % 2 == 0){ ?>
					<li>
						<div class="date-box">
							...
							<span class="date"><?php echo esc_attr(date(get_option('date_format'),strtotime($event->event_start_date)));?></span>
						</div>
						<div class="text-box"> 
							<strong class="title">
							<a href="<?php echo esc_url($event->guid);?>"><?php echo substr(esc_html($event->post_title),0,15);?></a></strong>
							<a class="time" href="<?php echo esc_url($event->guid);?>"><i class="fa fa-clock-o"></i><?php echo esc_attr(strtotime($event->event_start_time))?></a>
							<a class="location" href="<?php echo esc_url($event->guid);?>"><i class="fa fa-map-marker"></i><?php echo esc_attr($event->get_location()->name); ?></a>
						</div>
					</li>                      
			<?php }else{ ?>
					<li> 
			...
			<?php if($category_array <> ''){?><li class="btn_all_box"><a href="<?php echo esc_url(get_term_link(intval($category_id),'event-categories'));?>" class="c-link"><?php esc_html_e('View Events','crunchpress');?></a></li><?php }?>
			</ul>
		</div>
		<?php }else{ ?>
			<h4><?php esc_html_e('There is no Recent Post to Show','crunchpress');?></h4>
		 <?php
		}
