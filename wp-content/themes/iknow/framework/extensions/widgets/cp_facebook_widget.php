...
?>
  ...
  <label for="<?php echo esc_attr($this->get_field_id('pageurl')); ?>">
	  <?php esc_html_e('Page URL:','crunchpress');?> 
	  <input class="widefat" id="<?php echo esc_attr($this->get_field_id('pageurl')); ?>" size='40' name="<?php echo esc_attr($this->get_field_name('pageurl')); ?>" type="text" value="<?php echo esc_url($pageurl); ?>" />
	<br />
      <small><?php esc_html_e('Please enter your page url example: http://www.facebook.com/profilename OR','crunchpress');?> <br />
     <?php esc_html_e('https://www.facebook.com/pages/wxyz/123456789101112','crunchpress');?> 
	</small><br />
  </label>
  </p> 
  <p>
  <label for="<?php echo esc_attr($this->get_field_id('showfaces')); ?>">
	 <?php esc_html_e('Show Faces:','crunchpress');?> 
	  <select id="<?php echo esc_attr($this->get_field_id('showfaces')); ?>" name="<?php echo esc_attr($this->get_field_name('showfaces')); ?>" class="widefat">
			<option <?php if($showfaces == 'true'){echo 'selected';}?> value="true"><?php esc_html_e('Yes','crunchpress');?></option>
			<option <?php if($showfaces == 'false'){echo 'selected';}?> value="false"><?php esc_html_e('No','crunchpress');?></option>
      </select>
  </label>
  </p>  
  <p>
  <label for="<?php echo esc_attr($this->get_field_id('showstream')); ?>">
	  <?php esc_html_e('Show Stream:','crunchpress');?> 
	   <select id="<?php echo esc_attr($this->get_field_id('showstream')); ?>" name="<?php echo esc_attr($this->get_field_name('showstream')); ?>" class="widefat">
			<option <?php if($showstream == 'true'){echo 'selected';}?> value="true"><?php esc_html_e('Yes','crunchpress');?></option>
			<option <?php if($showstream == 'false'){echo 'selected';}?> value="false"><?php esc_html_e('No','crunchpress');?></option>
      </select>
  </label>
  </p> 
  <p>
  <label for="<?php echo esc_attr($this->get_field_id('likebox_width')); ?>">
	  <?php esc_html_e('Like Box Width:','crunchpress');?>
  ...
  <label for="<?php echo esc_attr($this->get_field_id('likebox_height')); ?>">
	  <?php esc_html_e('Like Box Height:','crunchpress');?>
  ...
  function update($new_instance, $old_instance)
  {
	...
	function widget($args, $instance)
	{
		...
		if (!empty($title))
			...
			<div class="fb-like-box" data-colorscheme="dark" data-href="<?php echo esc_url($pageurl);?>" data-width="<?php echo esc_attr($likebox_width);?>" data-height="<?php echo esc_attr($likebox_height);?>"  data-show-faces="<?php echo esc_attr($showfaces);?>" data-header="false" data-stream="<?php echo esc_attr($showstream);?>" data-show-border="false"></div>
			<div id="fb-root"></div>
