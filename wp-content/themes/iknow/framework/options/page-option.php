	...
	*/
	...
	$page_meta_boxes = array(
		...
		'page-caption'=>array(					
			...
			'name'=> 'page-option-item-page-caption',

			'type'=> 'inputtext',

			'default'=> '',
			
			'class'=> 'page-caption-here',

			'description'=>'Please Write Page caption here.'),
	...
	function add_page_option_element(){
		...
		$page_meta_boxes['Page Item']['name']['Popular-Posts']['category']['options'] = get_category_list_array( 'category' );
			...
			if( $edit_meta_box['type'] == 'page-option-item' ){
				...
				for($i=0; $i<$num; $i++){
					...
					if(!isset($item_content_num[$item_type_new])){
						...
						}else if($item_type_new == 'Division_Start'){

							$item_content_num['parallax-item'] = 0;
					...
					foreach($item_content as $key => $value){					
						...
						if($key == 'slider-item'){
							...
							for($j=0; $j<$tab_num; $j++){
								...
								$item_xml = $item_xml . '<parallax>';

								$temp = isset( $_POST[$value['parallax_z_index']][$item_content_num[$tab_type]] )? htmlspecialchars($_POST[$value['parallax_z_index']][$item_content_num[$tab_type]]) : 'abc';

								$item_xml = $item_xml . create_xml_tag('parallax_z_index', $temp);

								$temp = isset( $_POST[$value['parallax_position']][$item_content_num[$tab_type]] )? htmlspecialchars($_POST[$value['parallax_position']][$item_content_num[$tab_type]]) : 'abc';

								$item_xml = $item_xml . create_xml_tag('parallax_position', $temp);

								$temp = isset( $_POST[$value['parallax_upload']][$item_content_num[$tab_type]] )? $_POST[$value['parallax_upload']][$item_content_num[$tab_type]] : 'abc';

								$item_xml = $item_xml . create_xml_tag('parallax_upload', $temp);
  ...
  <?php
				...
				foreach( $name[$item_type] as $input_key => $input_value ){
					...
					if( $input_key == 'slider-item' ){
				      ...
				      }else if( $input_key == 'parallax-item' ){

							   print_box_parallax($input_value, find_xml_node($xml_val, 'parallax-item'));
...
<?php
	...
	function print_box_parallax($name, $values){

	

		?>
  <div class="meta-body">
    <div class="meta-title meta-tab"><?php _e('ADD MORE PARALLAX IMAGES','crunchpress');?></div>
    <div id="page-tab-add-more" class="page-tab-add-more" />
  </div>
  <br class="clear">
  <div class="meta-input">
    ...
    <div class="added-tab" id="added-tab">
      <ul>
        <li id="page-item-tab" class="default">
			<div class="meta_title_cp">
				<div class="meta-title meta-tab-title"><?php _e('Z-Index','crunchpress');?></div>
				<input type="text"  id='<?php echo $name['parallax_z_index']; ?>' />
				<p><?php _e('add z-index for the image eg: 200','crunchpres');?></p>
			</div>
			<div class="meta_title_cp">
				<div class="meta-title meta-tab-title"><?php _e('Position','crunchpress');?></div>
				<input type="text"  id='<?php echo $name['parallax_position']; ?>' />
				<p><?php _e('Vertical Position of Image','crunchpres');?></p>
			</div>
			<div class="meta_title_cp">
				<div class="meta-title meta-tab-title"><?php _e('TABS TEXT','crunchpress');?></div>
				<input type="hidden" id="upload_image_attachment_id" value="" />
				<input id="upload_image_text_meta" class="upload_image_text_meta" type="text" value="" />
        ...
        <li id="page-item-tab" class="page-item-tab">
          <div class="meta_title_cp">
				<div class="meta-title meta-tab-title"><?php _e('Z-Index','crunchpress');?></div>
				<input type="text" name='<?php echo $name['parallax_z_index']; ?>[]' id='<?php echo $name['parallax_z_index']; ?>' value="<?php echo find_xml_value($tab, 'parallax_z_index'); ?>" />
				<p><?php _e('add z-index for the image eg: 200','crunchpres');?></p>
			</div>
			<div class="meta_title_cp">
				<div class="meta-title meta-tab-title"><?php _e('Position','crunchpress');?></div>
				<input type="text" name='<?php echo $name['parallax_position']; ?>[]' id='<?php echo $name['parallax_position']; ?>' value="<?php echo find_xml_value($tab, 'parallax_position'); ?>" />
				<p><?php _e('Vertical Position of Image','crunchpres');?></p>
			</div>
			<div class="meta_title_cp">
				<div class="meta-title meta-tab-title"><?php _e('TABS TEXT','crunchpress');?></div>
				<input name='<?php echo $name['parallax_upload']; ?>[]' type="hidden" id="upload_image_attachment_id" value="<?php echo find_xml_value($tab, 'parallax_upload'); ?>" />
				<input id="upload_image_text_meta" class="upload_image_text_meta" type="text" value="<?php echo find_xml_value($tab, 'parallax_upload'); ?>" />
				<input class="upload_image_button_meta" type="button" value="Upload" />
			</div>
          <br>
          <div id="unpick-tab" class="unpick-tab"><i class="fa fa-remove-sign"></i></div>
