<?php
	...
	*/
	...
	function print_item_size($item_size, $addition_class=''){
		...
		switch($item_size){
			case 'element1-4':
				echo '<div class="span3 mbtm ' . $addition_class .' ' .$cp_item_row_size. '">';
				$cp_item_row_size += 1/4; 
				break;
			case 'element1-3':
				echo '<div class="span4 mbtm ' . $addition_class .' ' .$cp_item_row_size. '">';
				$cp_item_row_size += 1/3; 
				break;
			case 'element1-2':
				echo '<div class="span6 mbtm ' . $addition_class .' ' .$cp_item_row_size. '">';
				$cp_item_row_size += 1/2; 
				break;
			case 'element2-3':
				echo '<div class="span8 mbtm ' . $addition_class .' ' .$cp_item_row_size. '">';
				$cp_item_row_size += 2/3; 
				break;
			case 'element3-4':
				echo '<div class="span9 mbtm ' . $addition_class .' ' .$cp_item_row_size. '">';
				$cp_item_row_size += 3/4; 
				break;
			case 'element1-1':
				echo '<div class="span12 mbtm ' . $addition_class .' ' .$cp_item_row_size. '">';
				$cp_item_row_size += 1; 
				break;	
		}
	...
	function print_column_item($item_xml){
		$column_html = '<div class="'.find_xml_value($item_xml,'container_class').'">
			'.do_shortcode(html_entity_decode(find_xml_value($item_xml,'column-text'))).'
		</div>';
	...
	function print_div_item ( $item_xml ){
		...
		$opacity = find_xml_value($item_xml, 'opacity'); 
		$repeat = find_xml_value($item_xml, 'repeat'); 
		$bgattachment = find_xml_value($item_xml, 'background-attachment'); 
		$bg_position = find_xml_value($item_xml, 'bg_position'); 
		$paddingtop = find_xml_value($item_xml, 'padding-top'); 
		$paddingbottom = find_xml_value($item_xml, 'padding-bottom'); 
		$one_pos = find_xml_value($item_xml, 'image-parallax-one-pos'); 
		$img_id_one = find_xml_value($item_xml, 'image-parallax-one'); 
		$two_pos = find_xml_value($item_xml, 'image-parallax-two-pos'); 
		$img_id_two = find_xml_value($item_xml, 'image-parallax-two'); 
		$cp_class = find_xml_value($item_xml, 'add-section-class'); 
		...
		if($select_type == 'Plain'){
			//echo '<div class="container inner-container-cp" ><div style="background-color:'.$bgcolor.';" class="color_class_cp"><div class="row-fluid" style="padding-top:'.$paddingtop.';padding-bottom:'.$paddingbottom.';float:left;">';
			echo '<div style="padding-top:'.$paddingtop.';padding-bottom:'.$paddingbottom.';" class="color_class_cp '.$cp_class.'"><div style="float:left;width:100%;" class="full-width"><div class="container"><div class="row-fluid">';
		}else if($select_type == 'Background Color'){
			echo '<div style="padding-top:'.$paddingtop.';padding-bottom:'.$paddingbottom.';background-color:'.$bgcolor.';" class="color_class_cp student-profile '.$cp_class.'"><div class="bg_full_transparency" style="float:left;width:100%;opacity:'.$opacity.';background-image:url('.$image_url[0].');background-size:cover;background-attachment:'.$bgattachment.';background-repeat:'.$repeat.';"></div><div style="float:left;width:100%;" class="full-width"><div class="container"><div class="row-fluid">';
		}else{
			if($bgattachment == 'Parallax'){
				...
				if(!empty($one_image)){
					...
					$html_one_img = '<div class="bg1 parallax_class" style="background-image:url('.$one_image[0].')" data-0="background-position:0px 0px;" data-end="background-position:'.$one_pos.';">&nbsp;</div>';
				...
				echo '<div style="padding-top:'.$paddingtop.';padding-bottom:'.$paddingbottom.';background-color:'.$bgcolor.';" class="color_class_cp '.$cp_class.'">
					'.$html_one_img.'
					'.$html_two_img.'
				...
				<div style="float:left;width:100%;" class="full-width"><div class="container"><div class="row-fluid">';
			}else{
				echo '<div style="padding-top:'.$paddingtop.';padding-bottom:'.$paddingbottom.';background-color:'.$bgcolor.';" class="color_class_cp '.$cp_class.'"><div class="bg_full_transparency" style="float:left;width:100%;opacity:'.$opacity.';background-image:url('.$image_url[0].');background-size:cover;background-attachment:'.$bgattachment.';background-repeat:'.$repeat.';"></div><div style="float:left;width:100%;" class="full-width"><div class="container"><div class="row-fluid">';
			}
		}
	...
	$gallery_div_size_listing_class = array(
		...
		'Catalogue View' => array( 'class'=>'span12', 'class2'=>'span4','class3'=>'span3','size'=>array(570,300),'size2'=>array(350,350),'size3'=>array(150,150)),
	); 	
	...
	function print_gallery_item($item_xml){
		...
		if(!empty($header)){
			echo '<h2 class="heading">' . esc_html($header) . '</h2>';
											...
											for($i=$limit_start;$i<$limit_end;$i++) { 
												...
												<div class="cp-gallery-item">
													...
													<div class="caption">
														...
														<a href="<?php echo esc_url($image_full[0])?>" class="zoom" data-rel="prettyPhoto[gallery1]"><i class="fa fa-search"></i></a>														
													</div>
												</div>
					...
					if( !empty( $slider_xml_string ) ){
						$slider_xml_dom->loadXML($slider_xml_string);
							if($gallery_size == 'Catalogue View'){ 
								echo '<div class="cp-gallery-container">';							
							}else{
							...
							for($i=$limit_start;$i<$limit_end;$i++) { 
								...
								if($gallery_size == 'Catalogue View'){								
									if($single_col % 3 == 0){$first_class="first";}else{$first_class = '';}$single_col++;
										...
										if( $link_type == 'Link to URL' ){
											...
											<div class="caption">
												...
												<a href="<?php echo esc_url($link); ?>"><i class="fa fa-link"></i></a>
											</div>
										<?php }else if( $link_type == 'Lightbox' ){
											...
											<div class="caption">
												<h4><?php echo esc_html($title);?></h4>
												<p><?php echo esc_html($caption);?></p>
												<a href="<?php echo esc_url($image_full[0]);?>" data-rel="prettyPhoto[gallery1]"><i class="fa fa-search"></i></a>
											</div>
											<?php
									...
									if($counter_gal_element % $gal_counter == 0){ ?>		
										<div class="clear"></div>
										<!--LIST ITEM START-->									
										<div class="list_item margin-bottom first fadeInUpBig cp_load <?php echo esc_attr($gallery_class);?>">
											...
											if( $link_type == 'Link to URL' ){
												$link = find_xml_value( $children->item($i), 'link');
												echo '<img class="cp-gallery-image" src="' . esc_url($image_thumb[0]) . '" alt="' . esc_attr($alt_text) . '" />'; ?>
											...
											<?php }else if( $link_type == 'Lightbox' ){
												...
												<?php if($title <> ''){ ?>
													...
													<div class="social-icons">
														...
														<a href="<?php echo esc_url($image_full[0]);?>" data-rel="prettyPhoto[gallery1]"><i class="fa fa-picture-o"></i></a>
													</div>	
												<?php }else{ ?>
													<img src="<?php echo esc_url($image_thumb[0]);?>" alt="<?php echo esc_attr($alt_text);?>">
													<div class="social-icons">
														<a href="<?php echo esc_url($image_full[0]);?>" data-rel="prettyPhoto[gallery1]"><i class="fa fa-plus"></i></a>
													</div>
												<?php } ?>
												<?php
											}else if( $link_type == 'Video' ){
									...
									<?php 
										...
										<!--LIST ITEM START-->									
										<div class="list_item margin-bottom fadeInUpBig cp_load <?php echo esc_attr($gallery_class);?>">
											<div class="gallery-frame">
											<?php 
											$image_full = wp_get_attachment_image_src($thumbnail_id, 'full');
											$image_thumb = wp_get_attachment_image_src($thumbnail_id, $item_size);
											...
											<?php }else if( $link_type == 'Lightbox' ){
												...
												<?php if($title <> ''){ ?>
													...
													<div class="social-icons">
														...
														<a href="<?php echo esc_url($image_thumb[0]);?>" data-rel="prettyPhoto[gallery1]"><i class="fa fa-picture-o"></i></a>
													</div>	
												<?php }else{ ?>
													<img src="<?php echo esc_url($image_thumb[0]);?>" alt="<?php echo esc_attr($alt_text);?>">
													<div class="social-icons">
														<a href="<?php echo esc_url($image_thumb[0]);?>" data-rel="prettyPhoto[gallery1]"><i class="fa fa-plus"></i></a>
													</div>
												<?php } ?>
												<?php
											}else if( $link_type == 'Video' ){
	...
	function print_newest_album_item($item_xml){
		...
		<?php if($header <> ''){ ?><h2 class="h-style"><?php echo esc_html($header);?></h2><?php }?>
						...
						<div class="row-fluid">
							<?php if($category_product <> ''){ ?>
								<div class="span3 new-album-cap fadeInLeft cp_load">
									...
									<p><?php echo substr(esc_html($category_term->description),0,200);?></p>
								</div>
							<?php }else{ ?>
								<div class="span12">
									<h2><?php esc_html_e('All Categories','crunchpress');?></h2>
								</div>	
							<?php } ?>
								...
								<?php
									...
									if(have_posts()){
										...
										<div class="slide"> 
											...
											<div class="item-cap"> <strong class="item-title"><a href="<?php echo esc_url(get_permalink());?>"><?php echo esc_html(get_the_title());?></a></strong> <span class="price-rating"> <span class="price"><?php echo esc_attr($currency);?><?php if($sale_price <> ''){echo esc_attr($sale_price);}else{echo esc_attr($regular_price);}?></span></span> 
												<form enctype="multipart/form-data" method="post" class="cart" action="<?php echo esc_url(get_permalink());?><?php echo esc_attr($permalink_structure);?>add-to-cart=<?php echo $post->ID;?>">
													...
													<button class="add_to_cart_button button product_type_simple added" data-quantity="1" data-product_sku="<?php echo $sku_num;?>" data-product_id="<?php echo $post->ID;?>" type="submit"><?php esc_html_e('Add to cart','crunchpress');?></button>
												</form>
											</div>
										</div>
	...
	function print_blog_item_item($item_xml){
		...
		<div class="blog_class" id="blog_store">
			<figure id="blog" class="span12 first">
				<?php if($header <> ''){?><h2 class="title"><?php echo esc_html($header);?><span class="h-line"></span></h2><?php }?>
				<div id="slider_blog">
							...
							<div class="content span8">
								...
								<div class="post_excerpt">
									...
									if($num_excerpt <> ''){
										...
										echo strip_tags(substr(esc_html(get_the_content()),0,250));
									}
									
									?></p>
									<a href="<?php echo esc_url(get_permalink());?>"><?php esc_html_e('Read More','crunchpress');?><i class="icon-plus"></i> </a>
								</div>
							</div>
						</li>
					<?php }
			...
			<div id="inner">
				...
				<?php
					...
					?>">
						...
						<a class="view_detail" href="<?php echo esc_html(get_permalink());?>"><?php esc_html_e('View Detail','crunchpress');?></a>
					</div>
				<?php }?>
				</div>
				<div id="pager">
	...
	function print_default_content_item(){
		...
		<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
			<?php
				the_title( '<h2 class="heading"><a href="'.esc_url(get_permalink()).'">', '</a></h2>' );
			?>
	...
	function print_accordion_item($item_xml){
		...
		$header = find_xml_value($item_xml, 'header');
		if(!empty($header)){
			echo '<div class="heading-bg"><h2 class="h-style">' . esc_html($header) . '</h2></div>';
		}
		...
		if($counter_accor == 0){$counter_accordion = 'open_first_1';}else{$counter_accordion = 'accordion-'.$counter_accor;}$counter_accor++;
			echo '<div class="custom_accordion_cp" id="'.$counter_accordion.'"><p>'.find_xml_value($accordion, 'title').'</p><span><i class="fa fa-minus"></i></span></div>';
			echo '<div class="container_cp_accor"><div class="content_cp_accordian"><p>';
			echo do_shortcode(html_entity_decode(find_xml_value($accordion, 'caption'))) . '</p></div></div>';
		}
	...
	function print_toggle_box_item($item_xml){
		...
		$header = find_xml_value($item_xml, 'header');
		if(!empty($header)){
			echo '<h3 class="toggle-box-header-title title-color cp-title">' . esc_html($header) . '</h3>';
		}
		...
		foreach($tab_xml->childNodes as $toggle_box){
			...
			echo "' ></span>";
			echo "<h3 class='color'>". find_xml_value(esc_html($toggle_box, 'title')) . "</h3>";
	...
	function print_tab_item($item_xml){
		...
		if( !empty($tab_widget_title) ){
			if(is_front_page()){
				...
				echo  esc_html($tab_widget_title);
	...
	function print_column_service($item_xml){		
		...
		$service_layout = find_xml_value($item_xml, 'style');
		$morelink = find_xml_value($item_xml, 'morelink');
		if($morelink <> ''){$html_readmore = '<a class="readmore" href="'.$morelink.'">'.esc_html__('Readmore','crunchpress').'</a>';}
		
		//$thumbnail = wp_get_attachment_image_src( $image , array(300,110) );
		if($service_layout == 'Style 1'){
			echo '<div class="services">
				<div class="header">
					<i class="fa fa-tablet"></i>
					<i class="'.esc_attr($fontaw).' inner-icon"></i>
				</div>
				<div class="text">
					<h3><a href="'.esc_url($morelink).'">'.esc_html($title).'</a></h3>
					<p>'.do_shortcode(html_entity_decode(esc_html($descrip))).'</p>
				</div>
			</div>';
		}else if($service_layout == 'Style 3'){
			echo '<div class="adminstrative">
				<h2><i class="'.esc_attr($fontaw).'"></i> <a href="'.esc_url($morelink).'">'.esc_html($title).'</a></h2>
				<p>'.do_shortcode(html_entity_decode(esc_html($descrip))).'</p>
			</div>';
		}else{
			echo '<div class="features service_div_sec">
				<i class="'.esc_attr($fontaw).'"></i>
				<h4><a href="'.esc_url($morelink).'">'.esc_html($title).'</a></h4>
				<p>'.do_shortcode(html_entity_decode(esc_html($descrip))).'</p>
			</div>';
	...
	function print_contact_form($item_xml){
		...
		<script type="text/javascript">
			...
			jQuery(document).ready(function($) {
				...
				// jQuery('#submit_btn').click(function () {
					//var now_loading = jQuery(this).parent().parent().find('#import-now-loading').val();
					
					
					//now_loading.fadeIn();
					// jQuery.post(ajaxurl, {
						// action: 'cp_contact_submit',
		...
		?>
		<div id="frm_area" class="leave-reply">					
			<form class="contact-form" id="form_contact" method="POST">
				<div id="succ_mess"></div>
				<ul class="frm_area">
					<li class="user">
						<!--<label><?php esc_html_e('Name','crunchpress');?></label>-->
						<input type="text" class="required require-field detail-input" value="" id="name_contact" name="name_contact" placeholder="Name"/>
					</li>
					<li class="mail">
						<!--<label><?php esc_html_e('Email','crunchpress');?></label>-->
					...
					<li class="web">
						<!--<label><?php esc_html_e('Website','crunchpress');?></label>-->
						<input type="text" class="required url require-field detail-input" value="" id="website" name="website" placeholder="Website"/>
					</li>
					<li>
						<!--<label><?php esc_html_e('Message','crunchpress');?></label>-->
						<textarea class="required require-field detail-textarea" id="message_comment" name="message_comment" cols="10" rows="10"></textarea>
						<input type="submit" id="submit_btn" class="detail-btn-sumbit2" value="<?php echo esc_html__('Submit','crunchpress'); ?>" /> 
					</li>
				</ul>						
				<div id="loading_div" class=""></div>
				<div class="hide"><input type="hidden" id="receiver" name="receiver" value="<?php echo find_xml_value($item_xml, 'email'); ?>" /></div>
				...
				while( have_posts() ){
					...
					<li> 
						...
						<div class="span7 ns_desc"> 
							...
							<a href="<?php echo esc_url(get_permalink())?>" class="rm"><?php esc_html_e('View All News &nbsp;','crunchpress');?><i class="fa fa-plus"></i></a>
						</div> 
					</li>
					<?php } //End while loop
					wp_reset_postdata();
	...
	function print_text_widget($item_xml){
		...
		echo ' mt0"><h3 class="text-widget-title">' . esc_html($title) . '</h3>';
		echo '<div class="text-widget-caption">' . esc_html(do_shortcode($caption)) . '</div>';
		echo '</div>';
		if( !empty($button_title) ){
			...
			echo '<a class="text-widget-button" style="position:relative; top:' . $button_margin . 'px;" href="' . esc_url(find_xml_value($item_xml, 'button-link')) . '" >';
	...
	function print_page_item($item_xml){		
		...
		while( have_posts() ){
			...
			if( !empty($thumbnail[0]) ){
				...
				echo '<a href="' . esc_url($permalink) . '" ' . esc_attr($pretty_photo) . ' title="' . esc_html(get_the_title()) . '">';
				echo '<span class="portfolio-thumbnail-image-hover">';
				echo '<span class="' . esc_html($hover_thumb) . '"></span>';
				echo '</span>';
				echo '</a>';
				echo '<img src="' . esc_url($thumbnail[0]) .'" alt="'. esc_html($alt_text) .'"/>';
			...
			if( find_xml_value($item_xml, "show-title") == "Yes" ){
				echo '<h2 class="heading portfolio-thumbnail-title port-title-color cp-title"><a href="' . esc_url(get_permalink()) . '">' . esc_html(get_the_title()) . '</a></h2>';
			}
			// page excerpt
			if( find_xml_value($item_xml, "show-excerpt") == "Yes" ){			
				echo '<div class="portfolio-thumbnail-content">' . mb_substr( esc_html(get_the_excerpt()), 0, $num_excerpt ) . '</div>';
			}
			// read more button
			if( find_xml_value($item_xml, "read-more") == "Yes" ){
				echo '<a href="' . esc_url(get_permalink()) . '" class="portfolio-read-more cp-button">' . esc_html__('Read More','crunchpress') . '</a>';
			}
			echo '</div>';
	...
	<section id="donation_box">	
		<div class="donation_box">
			...
			<figure class="span2">
					<a href="<?php echo esc_url($button_link);?>" class="donate-now btn btn-large dropdown-toggle" type="submit"><?php echo esc_html($donate_button);?></a>
			</figure>
		</div>
	</section>
	<?php }
		...
		<div id="charity_progress">
			...
			<div id="charity_process_inner">
				<div class="span4 img first">
					<img src="<?php echo esc_url($ign_product_image1);?>" alt="<?php echo esc_html(get_the_title($project));?>"/>
				</div>
				<div class="span8 progress_report">
				<h2> <?php esc_html_e('$','crunchpress');?><?php echo esc_html(getTotalProductFund_cp($ign_project_id));?> </h2>
				<h4><?php esc_html_e('Pledged of','crunchpress');?> <?php esc_html_e('$','crunchpress');?><?php echo esc_attr($ign_fund_goal);?> <?php esc_html_e('Goal','crunchpress');?></h4>
					<div class="progress progress-striped active">  
						<div style="width:<?php echo esc_attr(getPercentRaised_cp($ign_project_id));?>%;" class="bar p80"></div>    
					</div>
					  <div class="info"> 
							<div class="span6 first">
								<i class="fa fa-user"></i> <span> <?php echo esc_attr($getPledge_cp[0]->p_number);?></span> <?php esc_html_e('Pledgers','crunchpress');?>
							</div>
							<div class="span6 ntr">
								<i class="fa fa-calendar-empty"></i> <span> <?php echo esc_attr($days);?></span> <?php esc_html_e('Days Left','crunchpress');?>
							</div>
					  </div>
				</div>
			</div>
			...
			<div class="slider_shop" id="slider_shop">
				...
				<?php
					...
					<li> 
						...
						<div class="price_cart"><span class="price"><?php echo esc_attr($currency);?><?php echo esc_attr($sale_price);?></span><a href="<?php echo esc_url(get_permalink());?>&add-to-cart=<?php echo $post->ID;?>"><i class="fa fa-shopping-cart"></i></a></div>
					</li>
					<?php }
					wp_reset_postdata();
					?>
	...
	function news_bar_frontpage($news_button,$news_title,$news_category){
		...
		//News Title
		if($news_title <> ''){ ?><strong class="news-title"><?php echo esc_html($news_title);?></strong><?php }else{ ?><strong class="news-title"><?php esc_html_e('Dont miss','crunchpress');?></strong><?php } ?>
				...
				foreach($post_list as $post){ ?>
						<div class="slide">
							<p><?php echo esc_html($post->post_title);?> 
								<em><?php echo htmlspecialchars(strip_tags(substr(esc_html($post->post_content),0,120)));?></em>
							</p>
						</div>
					<?php
