<?php
	...
	*/ 
	...
	function register_meta_script(){
		...
		// register style and script when access to the "testimonial" post_type page		
		}else if( $post_type == 'courses' || $post_type == 'lessons' || $post_type == 'question' ){
		
			wp_register_script('cp-lesson-metadata', CP_PATH_URL.'/framework/javascript/assets/js/lesson-metadata.js', false, '1.0', true);
			wp_enqueue_script('cp-lesson-metadata');
			
			wp_register_script('cp-lesson-chosen', CP_PATH_URL.'/framework/javascript/assets/chosen/chosen.jquery.js', false, '1.0', true);
			wp_enqueue_script('cp-lesson-chosen');
			
			wp_register_script('cp-chosen-ajax', CP_PATH_URL.'/framework/javascript/assets/chosen/ajax-chosen.jquery.js', false, '1.0', true);
			wp_enqueue_script('cp-chosen-ajax');		

			wp_enqueue_style('cp-edu-settings-css',CP_PATH_URL.'/framework/stylesheet/education/settings.css');			

			// Localise script
			$translation_strings = array( 'wrong_colon' => __( 'Wrong:', 'crunchpress' ), 'add_file' => __( 'Add file', 'crunchpress' ), 'change_file' => __( 'Change file', 'crunchpress' ), 'confirm_remove' => __( 'Are you sure you want to remove this question?', 'crunchpress' ), 'confirm_remove_multiple' => __( 'Are you sure you want to remove these questions?', 'crunchpress' ), 'too_many_for_cat' => __( 'You have selected more questions than this category contains - please reduce the number of questions that you are adding.', 'crunchpress' ) );
			...
			wp_localize_script( 'cp-lesson-metadata', 'woo_localized_data', $data );

			// Chosen RTL
			if ( is_rtl() ) {
				wp_enqueue_script( 'cp-chosen-rtl', CP_PATH_URL . '/framework/javascript/assets/chosen/chosen-rtl.js', array( 'jquery' ), '1.0', true );
			}
	...
	*/ 
	...
	function register_non_admin_styles(){
		...
		wp_enqueue_style('cp_widget_default',CP_PATH_URL.'/frontend/css/cp_widgets_default.css'); //Include Default Widgets and Other settings
		//wp_enqueue_style('cp_color_default',CP_PATH_URL.'/frontend/css/color.css'); //Include Default Widgets and Other settings
		wp_enqueue_style('cp_woo_default',CP_PATH_URL.'/frontend/css/wp-commerce.css'); //Include Default Widgets and Other settings
		...
		wp_enqueue_style('cp-owl',CP_PATH_URL.'/frontend/css/owl.carousel.css'); //Event Manager		
		...
		//Widget Gallery
		if(is_active_widget( '', '', 'gallery_image_show')){			
		...
		// Page post_type
			...
			if( strpos($cp_page_xml,'<Sermons>') > -1 || strpos($cp_page_xml,'<Gallery>') > -1 || strpos($cp_page_xml,'<News>') > -1 || strpos($cp_page_xml,'<Events>') > -1){
				wp_enqueue_style('prettyPhoto',CP_PATH_URL.'/frontend/css/prettyphoto.css');				
			}
	...
	function register_non_admin_scripts(){
		...
		wp_register_script('cp-modernizr', CP_PATH_URL.'/frontend/js/modernizr.js', false, '1.0', true);
		...
		//Custom Script Loaded
		wp_register_script('cp-owl', CP_PATH_URL.'/frontend/js/owl.carousel.js', false, '1.0', true);
		wp_enqueue_script('cp-owl');
		...
		// Page post_type
			...
			if( strpos($cp_page_xml,'<Accordion>') > -1 ){
				wp_register_script('cp-accordion-script', CP_PATH_URL.'/frontend/js/jquery.accordion.js', false, '1.0', true);
				wp_enqueue_script('cp-accordion-script');
			...
			//Gallery Portfolio, News, Events Elements
			...
			if( strpos($cp_page_xml,'<Division_Start>') > -1){				
				wp_register_script('cp-skrollr', CP_PATH_URL.'/frontend/js/skrollr.min.js', false, '1.0', true);
			...
			//Bx Slider Scripts Client-Slider 
			if(strpos($cp_page_xml,'<slider-type>Bx-Slider</slider-type>') > -1 || strpos($cp_page_xml,'<Client-Slider>') > -1){
				wp_register_script('cp-bx-slider', CP_PATH_URL.'/frontend/js/bxslider.min.js', false, '1.0', true);
				wp_enqueue_script('cp-bx-slider');		
			...
			if( strpos($cp_page_xml,'<Accordion>') > -1 ){
				wp_register_script('cp-accordian-script', CP_PATH_URL.'/frontend/js/jquery.accordion.js', false, '1.0', true);
			...
			// If using filterable plugin
			if( strpos($cp_page_xml,'<Topics>') > -1 ){
